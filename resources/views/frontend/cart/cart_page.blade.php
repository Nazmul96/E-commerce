                                ...
                                @foreach ($content as $row)
                                    ...
                                    <li class="cart_item clearfix">
                                        ...
                                        <div class="cart_item_image">
                                            <img src="{{ asset('public/files/product/' . $row->options->thumbnail) }}"
                                                alt="">
                                        </div>
                                        <div class="cart_item_info d-flex flex-md-row flex-column justify-content-between">
                                            ...
                                            @if ($row->options->size != null)
                                                <div class="cart_item_color cart_info_col">
                                                    <div class="cart_item_text">
                                                        <select class="custom-select form-control-sm size_change" name="size"
                                                            style="min-width: 100px;" data-id="{{ $row->rowId }}">
											...
											<div class="cart_item_color cart_info_col">
												<div class="cart_item_text">
													<select class="custom-select form-control-sm color_change" data-id="{{ $row->rowId }}" name="color" style="min-width: 100px;">
                                            ...
                                            <div class="cart_item_quantity cart_info_col">
                                                <div class="cart_item_text">
                                                    <input type="number" class="form-control-sm quantity_increase" name="qty" style="min-width: 70px;" data-id="{{ $row->rowId }}"  value="{{ $row->qty }}" min="1" required="">
...
<script>
	...
	$('body').on('blur','.quantity_increase', function(){
		    let qty=$(this).val();
	...
	//color update
	$('body').on('change','.color_change', function(){
		    let color=$(this).val();
			alert(color);
		    let rowId=$(this).data('id');
		    $.ajax({
	...
	//size update
	$('body').on('change','.size_change', function(){
	let size=$(this).val();
	let rowId=$(this).data('id');
	$.ajax({
